...
デフォルトでは、入力ファイルと同じフォルダに `[元のファイル名]_nobg.webm` として出力されます。

変更したい場合は「変更」ボタンをクリックして出力先を指定してください。
...
### 形式
- **WebM (VP9)**: アルファチャンネル（透過）付きの軽量形式
- **自動圧縮**: VP9コーデックにより大幅にファイルサイズを削減
- **音声**: 入力動画に音声がある場合、自動的に出力に含まれます（Opus形式）

### 対応ソフトウェア
出力されたWebMファイルは以下のソフトウェアで開けます：

- **Canva** - ドラッグ＆ドロップで透過動画として利用可能
- Adobe Premiere Pro
- Adobe After Effects
- DaVinci Resolve
- Final Cut Pro
