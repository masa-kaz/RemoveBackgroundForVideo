; 動画背景除去ツール - GPU版 Inno Setup スクリプト
;
; NVIDIA GPU検出機能付き
; - GPU検出時: インストール続行
; - GPU未検出時: インストール不可、終了を促す
...
; 出力設定
OutputDir=..\dist
OutputBaseFilename=BackgroundRemover_GPU_Setup
; インストーラーの圧縮設定
...
begin
  ...
  begin
    ...
    begin
      ...
      if ExistingVersion = '{#MyAppVersion}' then
        Msg := '動画背景除去ツール セットアップ (GPU版)' + #13#10 + #13#10 +
               '✅ NVIDIA GPU が検出されました' + #13#10 +
               '     ' + GPUName + #13#10 + #13#10 +
  ...
  begin
    // GPU未検出時 - インストール不可
    Msg := '動画背景除去ツール セットアップ (GPU版)' + #13#10 + #13#10 +
           '⚠️ NVIDIA GPU が検出されませんでした' + #13#10 + #13#10 +
           'このインストーラーはGPU版です。' + #13#10 +
           'CPU版をご利用ください。';
